# Message System 全量 Commit Review（163 commits）
...
#### `5a991ae` Initial commit: Message System - Real-time Chat System with Redis
...
#### `274f5ce` Update README to reflect project name as Message System
- 改动：仅 README/README.zh 各两行，统一项目名为 Message System。
- 评审：纯文档微调，名实相符。
- 问题：无明显问题
...
- 问题：[低] commit message 写的 key 名（message-system_last_room_id/last_view）与代码实际（message-system_current_room/current_view）不符；[低] getRoomById 无超时，服务端不回调时 isLoadingRoom 永久卡 true。
...
#### `9788d78` Merge branch 'master' of github.com:WENLIN-LI/message-system
...
**批次小结**：本批 27 个 commit 横跨两个阶段：2025-09 的 v1.0 AI 助手首发（76c669b/32bd7e7 两个 2.5k+ 行 mega commit，流式管线能跑但埋下历史覆盖丢数据、消息级零鉴权、无频控三大结构性隐患），与 2026-05 的密集重构周（Message System 设计系统、5 语言 i18n 工具链、e10f4a0/204a683 server+client 模块化拆分并补 2k+ 行测试、OpenRouter→DeepSeek/Anthropic 官方 API 多 provider 接入与成本核算）。后期工程质量显著提升（纯函数拆分、fake Redis 测试、AST lint），但 manualChunks TDZ 四连修（17ff0e0→823f344→69f6925→9cbd4cd 净产出为零）与 Fly CI 三连修暴露"不在本地验证、靠线上报错驱动迭代"的交付习惯；个别 commit message 名实不符（026bbea）。批末 saveMessageHistory 覆盖竞态与 edit/delete 无鉴权仍未解决。
...
#### `76c669b` feat: add streaming AI assistant and refresh Message System UI
...
#### `a9ff175` feat(ui): add Message System design foundation
...
- 改动：录音手势遮罩从黑底微信绿改成品牌色（含 safe-area-inset 底栏）、HeroUI `<Image>` 换原生 `<img>` 并抽出 `imageMedia` 节点、`<audio>` 用 `.message-system-audio-player` CSS 主题化 webkit 控件，补一条渲染测试。
...
- 改动：appViewport 在 visualViewport 缩小超 120px 且焦点在可编辑元素时给根元素挂 `message-system-keyboard-open` 类，CSS 借此隐藏移动端底部导航；cleanup 时移除类。
...
- 改动：给各 Modal wrapper 加 `message-system-modal-viewport` CSS 类，用既有 `--app-height`/`--app-viewport-top` 变量把弹层钉在可见视口内；发帖时段编辑器用时/分两个 Select 替换 `TimeInput`，避免唤起数字键盘。
