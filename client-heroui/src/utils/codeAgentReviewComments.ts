...
export function hasReviewCommentMessageSegments(value: string): boolean {
  return parseReviewCommentMessageSegments(value).some((segment) => segment.kind === 'review-comment');
}
...
function formatDiffReviewRangeLabel(lines: ReadonlyArray<DiffReviewLine>): string {
  ...
  const marker = firstMarker.length > 0 && lines.every((line) => line.change === firstLine.change)
    ? firstMarker
    : '';
