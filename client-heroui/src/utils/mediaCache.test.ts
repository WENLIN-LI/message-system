...
describe("mediaCache", () => {
    ...
    vi.stubGlobal("caches", {
      open: vi.fn(async (name: string) => {
        ...
        return {
          ...
          keys: vi.fn(async () => [...store.keys()].map(key => new Request(`https://message-system.local${key}`))),
  ...
  it("returns cached media object URLs without fetching", async () => {
    ...
    bodyCache.set(
      "/message-system-media-cache/body/cached-image-asset",
      new Response(new Blob(["image"], { type: "image/webp" }), { status: 200 }),
    );
    cachesByName.set("message-system-media-body-v1", bodyCache);
  ...
  it("returns cached video posters without reading the video again", async () => {
    ...
    posterCache.set(
      "/message-system-media-cache/poster/video-asset.jpg",
      new Response(new Blob(["poster"], { type: "image/jpeg" }), { status: 200 }),
    );
    cachesByName.set("message-system-video-poster-v1", posterCache);
