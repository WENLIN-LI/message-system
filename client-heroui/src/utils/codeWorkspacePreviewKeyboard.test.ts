...
import { makeCodeWorkspacePreviewAutomationKeySequence } from './codeWorkspacePreviewKeyboard';

describe('codeWorkspacePreviewKeyboard', () => {
  it('includes the Chromium virtual key code and Enter text', () => {
    expect(makeCodeWorkspacePreviewAutomationKeySequence({ key: 'Enter' })).toEqual({
      keyDown: {
        type: 'keyDown',
        key: 'Enter',
        code: 'Enter',
  ...
  it('dispatches printable keys as text key-down events', () => {
    const sequence = makeCodeWorkspacePreviewAutomationKeySequence({ key: 'z' });
  ...
  it('suppresses text and uses raw key-down for shortcuts', () => {
    expect(
      makeCodeWorkspacePreviewAutomationKeySequence({ key: 'a', modifiers: ['Meta'] }, { isMac: true }).keyDown,
    ).toEqual({
      type: 'rawKeyDown',
      key: 'a',
      code: 'KeyA',
  ...
  it('maps common macOS editing shortcuts without changing other platforms', () => {
    expect(
      makeCodeWorkspacePreviewAutomationKeySequence({ key: 'z', modifiers: ['Shift', 'Meta'] }, { isMac: true })
        .keyDown.commands,
    ).toEqual(['redo']);
    expect(
      makeCodeWorkspacePreviewAutomationKeySequence({ key: 'a', modifiers: ['Meta'] }).keyDown,
    ).not.toHaveProperty('commands');
  });

  it('resolves shifted printable keys to their browser values', () => {
    const sequence = makeCodeWorkspacePreviewAutomationKeySequence({ key: '1', modifiers: ['Shift'] });
    expect(sequence.keyDown).toMatchObject({
      key: '!',
      code: 'Digit1',
      modifiers: 8,
