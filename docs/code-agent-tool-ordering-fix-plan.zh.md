...
Message System 现在的 Code Agent 链路分成四段：

1. 浏览器发起 Code Agent turn。
2. Node `CodeAgentSessionService` 创建 AI placeholder，并按收到的 runner 事件追加消息。
3. Python `message-system_code_agent_runner` 调用 Code Agent `Engine.run(...)`。
...
- Message System runner 可以不改 Code Agent engine，直接扫描最终 messages 生成 Message System 的 `tool_call/tool_result`。
...
| 层 | 应该负责 | 不应该负责 |
|---|---|---|
| Code Agent engine | 按真实执行顺序暴露 text/tool call/tool result 边界 | Message System 的 DB position、Socket 事件 |
| Message System Runner | 把 Code Agent ordered events 转成 JSONL 协议 | 事后猜 AI 文本应该切在哪里 |
| Message System Node | 按 JSONL 到达顺序落库、切 AI segment | 根据完整 AI 文本反推工具发生点 |
| Browser | 展示服务端给出的顺序，实时合并 chunk | 修正已经错误的 DB transcript |

结论：主修点应该是 **Code Agent engine + Message System Runner**。只改前端或只改 Node 都不够。
...
- `on_tool_event` 同时服务 Code Agent CLI 和 Message System runner。
...
- `server/message-system_code_agent_runner/message-system_code_agent_runner/runner.py`
- `server/message-system_code_agent_runner/tests/test_runner.py`
...
| 文件 | 改动 |
|---|---|
| `server/message-system_code_agent_runner/message-system_code_agent_runner/runner.py` | 接 `on_tool_event`，直接发 JSONL tool events，fallback 才 replay |
| `server/message-system_code_agent_runner/tests/test_runner.py` | 覆盖 live tool event 顺序、fallback replay、无重复事件 |
...
2. Message System 更新 `ops/code-agent-sandbox/artifact.lock.json` 里的 `codeAgentEngine.sourceRef` 和 artifact version。
3. 重新准备 sandbox context：

```bash
node scripts/code-agent/prepare-sandbox-context.mjs \
  --output /tmp/message-system-code-agent-sandbox-context
```
...
pytest server/message-system_code_agent_runner
```
...
这不是单纯前端问题，也不是 Node store 随机排序问题。根因是 Message System runner 使用了 Phase 6 MVP 的“文本实时、工具事后 replay”策略，而 Code Agent engine 没有给 Message System 暴露完整 ordered tool events。
