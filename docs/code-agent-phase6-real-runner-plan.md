...
Run the existing Code Agent coding assistant inside the file/process sandbox. Message System must not reimplement Code Agent's agent loop. Message System only owns:
...
```mermaid
flowchart LR
  RT["Message System CodeAgentSessionService"] --> SB["Sandbox Service"]
  SB --> RP["Runner Process"]
  RP --> CA["message-system_code_agent_runner"]
...
- Add a Python `message-system_code_agent_runner` package in the Message System repo.
