...
- 保留 Message System 里的 `coco` 房间类型、UI 名称、消息结构和 `CocoRunnerEvent` 协议。
...
- Codex task 运行时，Message System 把用户的 encrypted `auth.json` 注入 Coco engine 内的 `codex_cli` 私有运行目录；任务结束后取回 refreshed `auth.json` 并重新加密保存。
...
- wrapper 接收 Message System 的 run request，恢复本次运行的 Codex subscription `auth.json`，调用 `codex exec --json`，把 stdout JSONL 转成 Message System `CocoRunnerEvent`。
...
- `server/src/services/cocoRuntimeConfig.ts` 会在 `CODE_AGENT_BACKEND=codex` 时默认选择 `python -m message-system_coco_runner.codex_cli`。
...
- 生产启用前再用生产 gateway 路径做一次端到端房间级验证：用户发起 Coco room turn，Message System 注入短期 gateway token / Codex auth，E2B dual-cli template 完成任务并回写消息。

已验证：

- `scripts/coco/prepare-sandbox-context.mjs --output /tmp/message-system-coco-dual-cli-context-check` 通过。
- `docker build -t message-system-coco-dual-cli:local-poc /tmp/message-system-coco-dual-cli-context-check` 通过。
- Docker image 内 `codex --version` 输出 `codex-cli 0.142.5`，`message-system_coco_runner.runner` 与 `message-system_coco_runner.codex_cli` 均可 import。
- Docker image 内挂载本机 Codex ChatGPT subscription `auth.json` 后，`python -m message-system_coco_runner.codex_cli` 跑通真实 `codex exec --json`，输出 Coco JSONL `status` / `text_delta` / `final`，final answer 为 `codex smoke ok`。
- 裸容器 smoke 需要先初始化 `/workspace` git repo；生产 E2B 路径已有 `initializeWorkspaceVersionControl` 前置步骤。
- `node scripts/coco/build-e2b-template.mjs --dry-run --context /tmp/message-system-coco-dual-cli-context-check --template message-system-coco-2026-07-04-dual-cli-candidate --publish` 输出的 E2B create/publish 命令正确。
...
- 真实 E2B template `message-system-coco-2026-07-04-dual-cli-candidate` 已构建并 publish；E2B template ID 为 `rgeqrltyo3gwsx2ie22n`，发布名为 `realruitao/message-system-coco-2026-07-04-dual-cli-candidate`。
...
- 单测证明 Codex backend 会写入 auth secret、启动 `python -m message-system_coco_runner.codex_cli`、读取 refreshed auth、清理 secret 文件。
...
npm run build
cd message-system_coco_runner && PYTHONPATH=. pytest
cd ../.. && node scripts/coco/prepare-sandbox-context.mjs --output /tmp/message-system-coco-dual-cli-context-check
node scripts/coco/build-e2b-template.mjs --dry-run --context /tmp/message-system-coco-dual-cli-context-check --template message-system-coco-2026-07-04-dual-cli-candidate --publish
...
- `RUN_CODEX_E2B_SMOKE=true npm run smoke:codex:e2b` passed on published E2B template `message-system-coco-2026-07-04-dual-cli-candidate`。
- `RUN_COCO_E2B_SMOKE=true npm run smoke:coco:e2b` passed on published E2B template `message-system-coco-2026-07-04-dual-cli-candidate` with local plan-only/direct DeepSeek smoke env。
...
- `docker build -t message-system-coco-dual-cli:local-poc /tmp/message-system-coco-dual-cli-context-check` passed。
