...
Coco runs inside a file/process sandbox. When the sandbox can write files or run shell commands, Message System must not inject long-lived provider API keys into that process.
...
- `COCO_SCOPED_PROVIDER_KEY_AUDIT_ID` must map the external credential to a Message System room/turn.
