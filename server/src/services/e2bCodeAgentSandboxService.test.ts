...
describe('E2BCodeAgentSandboxService', () => {
  it('creates, starts commands, and destroys E2B sandboxes through the driver', async () => {
    ...
    const service = new E2BCodeAgentSandboxService(driver, {
      templateId: 'message-system-code-agent',
      artifactVersion: 'message-system-code-agent-2026-06-28-a4e70e6',
    ...
    assert.deepEqual(driver.commands, ['python -m message-system_code_agent_runner']);
  ...
  it('fails loudly when the driver cannot execute commands or kill sandboxes', async () => {
    ...
    await assert.rejects(() => service.startRunner({ handle, command: 'python -m message-system_code_agent_runner' }), /command execution/);
