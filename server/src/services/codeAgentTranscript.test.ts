...
describe('code-agent transcript projection', () => {
  it('projects Message System user, assistant, and tool messages into code-agent prior_messages', () => {
  ...
  it('groups adjacent tool calls and tool results in deterministic code-agent message blocks', () => {
    const transcript = buildCodeAgentPriorMessages([
      ...
      message({
        ...
        toolCallId: 'tool-2',
        toolName: 'Grep',
        toolArgs: { pattern: 'Message System' },
      }),
      message({
        id: 'result1',
        clientId: 'code_agent_runner',
    ...
    assert.deepEqual(transcript, [
      {
        ...
        content: [
          ...
          { type: 'tool_use', id: 'tool-2', name: 'Grep', input: { pattern: 'Message System' } },
        ],
      },
      {
        role: 'user',
        content: [
          { type: 'tool_result', tool_use_id: 'tool-1', content: '# Message System' },
