...
describe('FakeCodeAgentSandboxService', () => {
  it('creates, connects, starts runners, counts, and destroys fake sandboxes', async () => {
    ...
    assert.equal(runner.command, 'python -m message-system_code_agent_runner');
    assert.deepEqual(service.startedRunnerCommands, ['python -m message-system_code_agent_runner']);
  ...
  it('can fail the next requested operation for lifecycle tests', async () => {
    ...
    await assert.rejects(() => service.startRunner({ handle, command: 'python -m message-system_code_agent_runner' }), /startRunner failed/);
  });
