  ...
  it('persists and reads saved rooms independently from owned rooms', async () => {
    const pool = new ScriptedPool([
      ...
      },
      {
        rows: [roomRow()],
        assertCall(call) {
          assert.match(call.sql, /SELECT id, name, description, created_at, last_activity_at, creator_id, message_version, password_hash, posting_schedule FROM rooms WHERE id = \$1/);
